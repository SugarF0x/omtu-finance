...
export default defineComponent({
  props: {
    fn: {
      type: Function as PropType<(VA: number, r: number, n: number) => number>,
      required: true
    }
  },
  setup(props) {
    ...
    const isDisabled = computed(() => {
      return !(
        VA.value && !isNaN(Number(VA.value)) &&
    ...
    const calculate = () => {
      result.value = round(fn.value(Number(VA.value), Number(r.value), Number(n.value)))
    }
...
<template>
  <div>
    <v-text-field v-model="VA" :rules="rules" placeholder="VA" label="Взнос" />
